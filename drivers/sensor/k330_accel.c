...
#undef K330_ACCEL_DEBUG_REGISTERS
 ...
 */
#define DEFAULT_POWER_ON_SETTING (ODR400 | K330ACCEL_ALL_AXES)
					...
					const char *buf, size_t count)
{
	...
	if (onoff == ON && data->movement_recog_flag == OFF) {
		...
		err = k330_accel_read_xyz(data, &raw_data);
		accel_dbgmsg("raw x = %d, y = %d, z = %d\n",
...
static ssize_t k330_accel_odr_read(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct k330_accel_data *data = dev_get_drvdata(dev);
	...
	return sprintf(buf, "reg1:0x%x,reg2:0x%x,reg3:0x%x,odr:0x%x,fs:0x%x\n"\
		...
		"sm1_s0:0x%x,sm1_s1:0x%x,sm2_s0:0x%x,sm2_s1:0x%x\n",
		reg1, reg2, reg3, odr_data, fs_data,
		src1, src2, mask1, mask2, thresh1, thresh2,
		sm1_s0, sm1_s1, sm2_s0, sm2_s1);
}
#endif
...
#ifdef K330_ACCEL_DEBUG_REGISTERS
static DEVICE_ATTR(odr, S_IRUGO, k330_accel_odr_read, NULL);
#endif
	...
	, void *k330_accel_data_p)
{
	...
	/* INT1 disable */
	if (i2c_smbus_write_byte_data(data->client, K330ACCEL_CTRL_REG3,
		PM_OFF))
