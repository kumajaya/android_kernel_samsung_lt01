...
#define FW_DIRECTORY	"tsp_melfas/w/"
...
static int mms_ts_fw_load(struct mms_ts_info *info)
{
	...
	if (system_rev == 0 || system_rev == 1) {
		dev_err(&client->dev, "system_rev 0.0 or 0.1 & FW update skip\n");
		return ret;
	}
	...
	while (retries_isc--) {
		...
		if (info->fw_ic_ver == 0x00) {
			...
			ret = mms100_ISC_download_mbinary(client, REQ_FW, FORCED_UPDATE);
		} else
			ret = mms100_ISC_download_mbinary(client, REQ_FW, COMPARE_UPDATE);
