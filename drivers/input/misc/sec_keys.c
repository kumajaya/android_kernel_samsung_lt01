...
static DEVICE_ATTR(wakeup_keys, S_IWUSR | S_IRUGO, NULL, wakeup_enable);
...
static void zoomring_init(struct sec_keys_drvdata *ddata)
{
	...
	u32 _code, int state)
{
	...
	case KEY_ACT_TYPE_TWO_CH:
		if (ddata->buff_rkey.code == _code)
			printk(KERN_ERR "[sec_keys] ignore %s\n",
				key_info->name);
		else {
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
			printk(KERN_DEBUG "[sec_keys] %s %s\n",
				key_info->name,
				state ? "High" : "Low");
#endif
			if (ddata->buff_rkey.state != state)
				code = (code == KEY_CAMERA_JOG_L) ?
					KEY_CAMERA_JOG_R : KEY_CAMERA_JOG_L;

			if (code == ddata->buff_rkey.prev) {
				report_enable = true;
				ddata->buff_rkey.prev = KEY_RESERVED;
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				printk(KERN_DEBUG "[sec_keys] %s event\n",
				...
				ddata->buff_rkey.prev = code;
				report_enable = false;
			}
		}

		ddata->buff_rkey.code = _code;
		ddata->buff_rkey.state = state;
