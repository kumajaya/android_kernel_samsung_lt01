...
static const struct drime4_frmsizeenum preview_frmsizes[] = {
	{ DRIME4_PREVIEW_720P,		1280,	720,	0x00 },
	{ DRIME4_PREVIEW_1056X704,	1056,	704,	0x01 },
	{ DRIME4_PREVIEW_960X720,	960,	720,	0x02 },
	{ DRIME4_PREVIEW_704X704,	704,	704,	0x03 },
};
...
static const struct drime4_fraction ss_val[] = {
	...
	{1, 2500}, {1, 3200}, {1, 4000}, {1, 5000}, {1, 6400},
};
...
static irqreturn_t drime4_isp_d4_isr(int irq, void *dev_id)
{
	...
	cam_trace("************ noti interrupt ************ pin:%d,ctrl:%d\n",
		drime4_spi_get_noti_ctrl_pin(), state->noti_ctrl_status);
...
static int drime4_set_exif_info(struct v4l2_subdev *sd, char *buf, u8 size)
{
	int err = 0;
...
#if 1
	err = drime4_write_buf_res(sd, DRIME4_PACKET_DSP_SET,
		DRIME4_SET_EXIF_GPS_INFO, (size-8), &buf[8], 0);
	CHECK_ERR(err);
#endif
	cam_trace("X\n");
	return err;
...
#ifdef BURST_BOOT_PARAM
	if (state->first_boot_param)
		err = drime4_add_boot_b_param(sd, DRIME4_SET_MOVIE_FPS,
			1, movie_fps);
	else
#endif
		err = drime4_writeb_res(sd, DRIME4_PACKET_DSP_SET,
	...
	struct v4l2_control *ctrl)
{
	...
	if (rdata[5] < 0x0C) {
		...
		if (!state->done_3a_lock) {
			...
			FD = (state->smart_read[0] >> 30) & 0x1;
			OT = (state->smart_read[0] >> 31) & 0x1;
			Lv = (state->smart_read[0] >> 24) & 0xF;
			ISO = rdata[18] & 0x1F;
			AF = rdata[19] & 0x1;
			if (rdata[5] > 0x0E)
...
static int drime4_set_i_func_off(struct v4l2_subdev *sd, int val)
{
	int err = 0;
	cam_dbg("E, value %d\n", val);
...
static int drime4_factory_log_write(struct v4l2_subdev *sd, int val)
{
	...
	err = drime4_cmd_buf(sd, 0xA2, 2, &dummy_data[0], 1);
	CHECK_ERR(err);

	int_factor = drime4_wait_ap_interrupt(sd,
		DRIME4_ISP_TIMEOUT);
	if (int_factor) {
		err = drime4_spi_read_log_res(&read_val[0], 0, 1);
		if (err < 0)
...
static int drime4_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAMERA_i_FUNC_OFF:
		err = drime4_set_i_func_off(sd, ctrl->value);
		break;
...
d4_retry:
...
#endif
		err = drime4_spi_read_res(&read_val[0], 1, 0);
	...
	case DRIME4_NOTI_LIVEVIEW_START_DONE:
		ctrl->value = 8;
		ctrl->value64 = 1;
		/* set production mode after liveview start done */
		if (state->factory_running) {
			mutex_lock(&state->drime4_lock);
			err = drime4_writeb_res(sd, DRIME4_PACKET_DSP_CMD,
	...
	struct v4l2_ext_control *ctrl)
{
	...
	case V4L2_CID_EXIF_INFO:
		...
		mutex_lock(&state->drime4_lock);
		err = drime4_set_exif_info(sd, buf, size);
		mutex_unlock(&state->drime4_lock);
...
boot_start_check_retry:
	gpio_val = drime4_spi_get_noti_check_pin();
	/*if (system_rev > 7) {
		if (gpio_val && (retry_cnt < 100)) {
			msleep(50);
			retry_cnt++;
			goto boot_start_check_retry;
		}
		if (gpio_val) {
			cam_err("ISP Boot start failed. I2C SDA = %s\n",
				gpio_val ? "High" : "Low");
			return -1;
		}
	} else */{
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (system_rev > 7) {
		...
		gpio_request(GPIO_GSENSE_SCL_18V, "CAM_I2C_SDA_2");
		gpio_request(GPIO_GSENSE_SDA_18V, "CAM_I2C_SCL_2");
		drime4_spi_set_noti_ctrl_pin(0);
...
static int __devexit drime4_remove(struct i2c_client *client)
{
	...
	if (system_rev > 7) {
		...
		s3c_gpio_setpull(GPIO_CAM_SDA, S3C_GPIO_PULL_DOWN);
