...
int drime4_spi_read_log(u8 *buf, size_t len, const int rxSize)
{
	int k;
	int ret = 0;
	u32 count = len/rxSize;
	...
	for (k = 0; k < count; k++) {
		ret = spi_xmit_rx(&buf[rxSize*k], rxSize);
		pr_notice("drime4_spi_read_test k=%d/count=%d/buf[] = 0x%02x, 0x%02x, 0x%02x, 0x%02x \n", k, count, 
			 buf[rxSize*k], buf[rxSize*k+1], buf[rxSize*k+2], buf[rxSize*k+3]);
		msleep(10);
		if (ret < 0)
			return -EINVAL;
	}
	...
	if (extra != 0) {
		ret = spi_xmit_rx(&buf[rxSize*k], extra);
		pr_notice("drime4_spi_read_test extra=%d\n",extra);
		msleep(10);
		if (ret < 0)
			return -EINVAL;
	}
...
#if 0
	pr_err("drime4 noti ctrl pin[%d] writing[%d]\n",
		g_noti_ctrl_pin, g_spi_writing);
#endif
	return g_noti_ctrl_pin;
}
	...
	if (retry == 1) {
		...
		if (retry_cnt < 1000) {
			...
			pr_err("HSW PINCHECK 00 : ap_int = %d, d4_int = %d, noti = %d, scl = %d\n",
				...
				noti_gpio_val, g_noti_ctrl_pin);
#endif
			if (retry_cnt > 2 && ap_gpio_val == 0
				&& d4_gpio_val == 0
				&& state->isp.ap_issued == 1) {
				u8 rdata[500] = {0,};
				int err = 0;
				pr_err("HSW PINCHECK 01 : state->isp.ap_issued = %d\n",
					state->isp.ap_issued);
...
int drime4_spi_read_log_res(u8 *buf, int noti, int log)
{
	int ret = 0;
	int all_length;
	int i, cal_checksum, sum = 0;

	memset(d4_rdata_buf2, 0, sizeof(d4_rdata_buf2));

	ret = drime4_spi_read_log((char *)&d4_rdata_buf2[0], 2, 4);
	if (ret < 0) {
		pr_err("drime4_spi Read error: length read\n");
		goto exit_err;
	}

	all_length = d4_rdata_buf2[0] + ((d4_rdata_buf2[1] << 8) & 0xFF00);
	...
	ret = drime4_spi_read_log((char *)&d4_rdata_buf2[2], all_length-2, 4);
	if (ret < 0) {
		pr_err("drime4_spi Read error: read data\n");
		goto exit_err;
	}
	...
	cal_checksum = ((~sum)+1) & 0xFF;
	if (d4_rdata_buf2[all_length-1] != cal_checksum) {
		pr_err("drime4_spi Read checksum error: cal 0x%02x, read 0x%02x\n",
			cal_checksum, d4_rdata_buf2[all_length-1]);
		log = 1;
		/*ret = -1;*/
	}

	memcpy(buf, d4_rdata_buf2, all_length-3);
