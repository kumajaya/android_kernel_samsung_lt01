...
static int m9mo_make_CSV_rawdata_direct(struct v4l2_subdev *sd, int nkind)
{
	...
	if (nkind == V4L2_CID_CAMERA_FACTORY_DEFECTPIXEL) {
	...
	}else if (nkind == V4L2_CID_CAMERA_FACTORY_RESOLUTION_LOG) {
		cam_dbg("start, file path %s\n", M9MO_FACTORY_CSV_PATH);
		tempBuf = kmalloc(intram_unit, GFP_KERNEL);
		if (!tempBuf) {
			cam_err("failed to allocate memory\n");
			err = -ENOMEM;
			goto out;
		...
		err = m9mo_mem_read(sd, unit_default, start, tempBuf);
		memcpy(buf, tempBuf, unit_default);

		buf_size += unit_default;
		...
		err = m9mo_mem_read(sd, unit_default, start, tempBuf);
		memcpy(buf+buf_size, tempBuf, unit_default);
...
static int m9mo_set_PASM_mode(struct v4l2_subdev *sd, int val)
{
	...
	case MODE_FIREWORKS:
		...
		/* Set Still Capture F-Number Value  - 8.0 or 8.8 */
#ifdef CONFIG_MACH_GC2PD
		err = m9mo_writeb(sd, M9MO_CATEGORY_AE,
			M9MO_AE_EV_PRG_F_NUMBER, 0x88);
#else
		err = m9mo_writeb(sd, M9MO_CATEGORY_AE,
			M9MO_AE_EV_PRG_F_NUMBER, 0x80);
#endif
...
static int m9mo_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAMERA_FACTORY_RESOLUTION_LOG:
		...
		err = m9mo_make_CSV_rawdata_direct(sd, V4L2_CID_CAMERA_FACTORY_RESOLUTION_LOG);
		CHECK_ERR(err);
		break;
