...
#define DRIME4_CMD_i_FN_OFF				0x030B
...
extern int drime4_spi_read_log(u8 *buf, size_t len, const int rxSize);
extern int drime4_spi_write_packet(struct v4l2_subdev *sd, u8 packet_id, u32 len,
	u8 *payload, int log);
...
extern int drime4_spi_read_log_res(u8 *buf, int noti, int log);
