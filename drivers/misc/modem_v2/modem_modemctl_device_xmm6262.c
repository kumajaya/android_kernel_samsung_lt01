...
static irqreturn_t phone_active_irq_handler(int irq, void *_mc)
{
	...
	cp_dump_value = gpio_get_value(mc->gpio_cp_dump_int) == mc->cp_dump_int_polarity;

	mif_info("PA EVENT : reset =%d, pa=%d, cp_dump=%d(gpio:%d)\n",
			...
			struct modem_data *pdata)
{
	...
	mc->cp_dump_int_polarity = pdata->cp_dump_int_polarity;
