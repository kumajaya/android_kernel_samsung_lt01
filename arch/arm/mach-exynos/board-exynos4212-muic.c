...
static int muic_init_gpio_cb(int switch_sel)
{
	...
	if (pdata->set_gpio_muic_adc_sel)
		pdata->set_gpio_muic_adc_sel(MUIC_PATH_MUIC);
#endif /* CONFIG_MUIC_USE_MUIC_ADC_SWITCH */

#if defined(CONFIG_MUIC_USE_ADDITIONAL_VBUS_LDO)
	if (pdata->set_gpio_v_bus_ldo)
		ret = pdata->set_gpio_v_bus_ldo(MUIC_VBUS_ENABLE);
...
#if defined(CONFIG_MUIC_USE_ADDITIONAL_VBUS_LDO)
static int muic_set_gpio_v_bus_ldo(int ldo_en)
{
	const char *mode;
	int ldo_en_gpio = exynos4_muic_pdata.gpio_v_bus_ldo;
	int ldo_en_val;
	int ret;

	ret = gpio_request(ldo_en_gpio, "GPIO_V_BUS_LDO");
	if (ret) {
		pr_err("failed to gpio_request GPIO_V_BUS_LDO\n");
		return ret;
	}

	ldo_en_val = gpio_get_value(ldo_en_gpio);

	pr_info("%s: ldo_en(%d), GPIO_V_BUS_LDO(%d)=%c ->", __func__, ldo_en,
			ldo_en_gpio, (ldo_en_val == 0 ? 'L' : 'H'));

	switch (ldo_en) {
	case MUIC_VBUS_ENABLE:
		mode = "V_BUS_LDO_ENABLE";
		if (gpio_is_valid(ldo_en_gpio))
			gpio_set_value(ldo_en_gpio, GPIO_LEVEL_LOW);
		break;
	case MUIC_VBUS_DISABLE:
		mode = "V_BUS_LDO_DISABLE";
		if (gpio_is_valid(ldo_en_gpio))
			gpio_set_value(ldo_en_gpio, GPIO_LEVEL_HIGH);
		break;
	default:
		mode = "Error";
		break;
	...
	gpio_free(ldo_en_gpio);

	pr_info(" %s, GPIO_V_BUS_LDO(%d)=%c\n", mode, ldo_en_gpio,
			(ldo_en_val == 0 ? 'L' : 'H'));
...
#if defined(CONFIG_MUIC_USE_MUIC_ADC_SWITCH)
static int muic_set_gpio_muic_adc_sel(int muic_adc_sel)
{
	const char *mode;
	int muic_adc_sel_gpio = exynos4_muic_pdata.gpio_muic_adc_sel;
	int muic_adc_sel_val;
	int ret;

	ret = gpio_request(muic_adc_sel_gpio, "GPIO_MUIC_ADC_SEL");
	if (ret) {
		pr_err("failed to gpio_request GPIO_MUIC_ADC_SEL\n");
		return ret;
	}

	muic_adc_sel_val = gpio_get_value(muic_adc_sel_gpio);

	pr_info("%s: muic_adc_sel(%d), GPIO_MUIC_ADC_SEL(%d)=%c ->", __func__,
			muic_adc_sel, muic_adc_sel_gpio,
			(muic_adc_sel_val == 0 ? 'L' : 'H'));

	switch (muic_adc_sel) {
	case MUIC_PATH_MUIC:
		mode = "Path to MUIC_USB";
		if (gpio_is_valid(muic_adc_sel_gpio))
			gpio_set_value(muic_adc_sel_gpio, GPIO_LEVEL_HIGH);
		break;
	case MUIC_PATH_ADC:
		mode = "Path to AP_ADC";
		if (gpio_is_valid(muic_adc_sel_gpio))
			gpio_set_value(muic_adc_sel_gpio, GPIO_LEVEL_LOW);
		break;
	default:
		mode = "muic_adc_sel Error";
		break;
	}

	muic_adc_sel_val = gpio_get_value(muic_adc_sel_gpio);

	gpio_free(muic_adc_sel_gpio);

	pr_info(" %s, GPIO_MUIC_ADC_SEL(%d)=%c\n", mode, muic_adc_sel_gpio,
			(muic_adc_sel_val == 0 ? 'L' : 'H'));
...
#if defined(CONFIG_MUIC_USE_ADDITIONAL_VBUS_LDO)
	.gpio_v_bus_ldo		= GPIO_V_BUS_EN,
	.set_gpio_v_bus_ldo	= muic_set_gpio_v_bus_ldo,
#endif /* CONFIG_MUIC_USE_ADDITIONAL_VBUS_LDO */
...
#if defined(CONFIG_MUIC_USE_MUIC_ADC_SWITCH)
	.gpio_muic_adc_sel	= GPIO_USB_SEL2,
	.set_gpio_muic_adc_sel	= muic_set_gpio_muic_adc_sel,
#endif /* CONFIG_MUIC_USE_MUIC_ADC_SWITCH */
